...
const Dashboard = () => {
  ...
  const fetchPasswords = () => {
    fetch(
      ...
      .then((data) => {
        if (data) {
          setPasswords(
            data.map((password) => ({ ...password, revealed: false }))
          );
        }
      })
  ...
  const handleDelete = (passwordId) => {
    fetch(
      `https://lockbox-password-server.vercel.app/users/${userID}/passwords/${passwordId}`,
      {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
  ...
  const handleEdit = (passwordId) => {
    ...
    const passwordToEdit = passwords.find(
  ...
  const handleUpdate = (index, updatedPassword) => {
    fetch(
      `https://lockbox-password-server.vercel.app/users/${userID}/passwords/${updatedPassword.id}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {passwords.length > 0 ? (
              passwords.map((password, index) => (
                <div
                  key={password.id}
                  className="mb-6 bg-gray-100 rounded-lg shadow-md p-6 relative"
                >
                  <div>
                  ...
                  <div className="mt-4">
                    {editingPasswordId === password.id ? (
                      <input
                        ...
                        onChange={(e) =>
                          handleInputChange("pw", e.target.value)
                        }
                        className="border rounded w-full p-2 mt-1 text-gray-800"
                      />
                    ) : (
